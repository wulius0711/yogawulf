...
}); // 06:00 – 23:00 (inkl. 23:00 für Abreise)
...
export default function Step1Veranstaltung({ slug }: Props) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <div>
        ...
        <input
          ...
          placeholder="z.B. Yoga-Retreat, Teambuilding, Meditation-Wochenende"
          autoFocus
        />
      </div>
      ...
      <div
        style={{
          ...
          gap: "1rem",
          background: hasRange ? "var(--primary-tint)" : "var(--bg2)",
          border: `1px solid ${hasRange ? "var(--primary-dim)" : "var(--border)"}`,
      ...
      >
        <div>
          ...
          <div style={{ fontWeight: 600, fontSize: "0.95rem", color: hasRange ? "var(--primary)" : "var(--muted)" }}>
            {fmtDate(form.datumVon)}
          </div>
        </div>
        <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
        <div>
          ...
          <select
            value={form.zeitVon}
            onChange={(e) => setField("zeitVon", e.target.value)}
          >
        ...
        <div>
          ...
          <select
            value={form.zeitBis}
            onChange={(e) => setField("zeitBis", e.target.value)}
          >
            <option value="">Uhrzeit wählen</option>
            {HOURS.map((h) => (
              <option key={h} value={h}>{h} Uhr</option>
